;(function(window) {
  ...
  function _validateMessage(data) {
    ...
    if (data.type === "platform_launch" && typeof data.token !== "string") {
      return "platform_launch requires token string";
    }
  ...
  function _sendInitToUnity() {
    if (!_isReady || !_userData) {
      log("Cannot send init to Unity: user not logged in or no userData");
      return;
    }
  ...
  function _onPlatformMessage(event) {
    ...
    switch (data.type) {
      case "platform_launch":
        _launchToken = data.token;
        _loginWithPlatformToken(_launchToken);
        break;

      case "auth_complete":
        if (data.token) {
          _loginWithPlatformToken(data.token, function() {
            _sendToUnity({
              type: "auth_complete",
              userData: _userData,
              signatureData: _signatureData,
            });
            _emit("user_changed", _userData);
          });
        } else {
  ...
  function _loginWithPlatformToken(token, onSuccess, attempt) {
    attempt = attempt || 1;
    ...
    fetch(_config.ssoLoginUrl, {
      ...
      .then(function(data) {
        if (!data || !data.userData) {
          throw new Error("Invalid SSO response: no userData field");
        }
        ...
        _signatureData = data.signatureData || null;
      ...
      .catch(function(err) {
        ...
        if (attempt < maxRetries) {
          ...
          setTimeout(function() {
            _loginWithPlatformToken(token, onSuccess, attempt + 1);
          }, delay);
        } else {
